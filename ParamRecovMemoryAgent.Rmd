...
# force confidence to not be 0 or 1
d1$confidence <- ifelse(d1$confidence == 0, 0.01, d1$confidence)
d1$confidence <- ifelse(d1$confidence == 1, 0.99, d1$confidence)

# remove values that are conceptually irrelevant
d1 <- filter(d1, d1$confidence > 0)
# remove values that are conceptually irrelevant
d1 <- filter(d1, confidence > 0 & confidence < 0.5)

```


This chunk errors the chunk below is what produced the plot
```{r}
 recor_df1 <- NULL
   for (confidencerateLvl in unique(d1$confidence)){
     for (biasLvl in unique(d1$bias)){
       for (betaLvl in unique(d1$beta)){
 
     d <- d1 %>% 
         filter(confidence == confidencerateLvl & bias == biasLvl & beta == betaLvl) 
     
     print(paste0("bias = ", biasLvl, " beta = ", betaLvl, " confidence = ", confidenceLvl ))
      ...
      samples <- mod$sample(
        ...
        adapt_delta = 0.99,
             ...
             temp <- tibble(
                       biasEst = draws_df$bias,
                       biasTrue = biasLvl,
                       confidenceRateEst = draws_df$confidencerate,
                       confidenceRateTrue = confidencerateLvl,
                       betaEst = draws_df$beta,
                       betaTrue = betaLvl)
     
      if (exists("recor_df1")) { recor_df1 <- rbind(recor_df1, temp)} else{recor_df1 <- temp} 
...
recor_df1 <- NULL
   for (confidencerateLvl in unique(d1$confidence)){
     ...
     d <- d1 %>% 
         filter(confidence == confidencerateLvl) 
     
     print(paste0(" confidence = ", confidenceLvl ))
...
# plot recovered confidencerate
confidenceRate_p <- ggplot(param_recov_df, aes(confidenceRateTrue, confidenceRateEst)) +
  geom_point(alpha = 0.1) +
  geom_smooth() +
  theme_classic()+
  ggtitle("Confidencerate Recovered")+
    theme(plot.title = element_text(size = 20, face = "bold"))

# arrange the two plots
ggarrange(bias_p, beta_p, ncol = 1, nrow = 3)

```

```{r}
# plot recovered confidencerate
confidenceRate_p <- ggplot(recor_df1, aes(confidenceRateTrue, confidenceRateEst)) +
  geom_point(alpha = 0.1) +
  geom_smooth() +
  theme_classic()+
  ggtitle("Confidencerate Recovered")+
